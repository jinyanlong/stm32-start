...
##### 6.4	读写器专用指令(CLA=0x12)
###9:低频发送数据（读写器）
	说明:
		读写器低频发送数据给标签
    设备发送:
		INS=0x04
		PARAMS= DATA(2B)+COUNT(1B)
		说明： COUNT-数据发送的次数
			  DATA-为要发送的数据
			
|DATA|COUNT|说明|返回数据|
|-|-|-|-|
|低频发送的数据|数据发送的次数||2.4G返回数据，具体返回数据参考通讯协议（2.4G+低频）|
|0x80XX|n|天线校准|[内容长度(1B)+信道(1B)]+指令类别码(1B)(0xA1)+指令码(1B)(0x80)+系统ID(4B)+卡号(4B)+CPU_ID(4B)+天线ID(2B)+X轴调整值+Y轴调整值+Z轴调整值(0~31)|
|0x81XX|1|读取标签属性,XX 表示属性ID.|[内容长度(1B)+信道(1B)]+指令类别码(1B)(0xA1)+指令码(1B)(0x81)+系统ID(4B)+卡号(4B)+CPU_ID(4B)+属性ID(1B)+属性类型/长度(1B)+属性值(...)|
|0x82XX|1| 强制标签进入/退出2.4G接收状态,XX 为超时的秒数，超过这个间隔，标签自动退出设置状态|[内容长度(1B)+信道(1B)]+指令类别码(1B)(0xA1)+指令码(1B)(0x82)+系统ID(4B)+CPU_ID(4B)+卡号(4B)|
